...
import { BIERES_INVITES, displayName, findCouple, uidFromIdToken } from '@/lib/bieresInvite'

/** POST — membres du couple + liens en attente (modale de partage). */
export async function POST(req: Request) {
  ...
  if (couple) {
    const invSnap = await db.collection(BIERES_INVITES).where('coupleId', '==', couple.id).get()
