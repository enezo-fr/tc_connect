...
import { Copy, Check, Link2, Trash2, UserPlus, Share2, Heart, ShieldCheck, Search } from 'lucide-react'
...
export function DuoShareModal({ isOpen, onClose }: Props) {
  ...
  // Le vrai verrou est côté serveur (`/api/duo-invite/add-member` vérifie
  ...
  const handleRattacher = async (u: User) => {
    ...
    try {
      await call('/api/duo-invite/add-member', { uid: targetUid })
      setRecherche('')
      await refresh()
    } catch (e: any) {
