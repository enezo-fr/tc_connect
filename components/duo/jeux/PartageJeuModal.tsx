...
import type { DuoPartie, User } from '@/types'
import {
  Copy, Check, Link2, Share2, QrCode, ShieldCheck, Search, UserPlus, Power, Mail, X, Trash2, Users,
} from 'lucide-react'
 ...
 *
 * Même architecture que le partage de belote : c'est la porte « sans compte » qui
 * fait tout l'intérêt un soir de jeux, quand personne autour de la table n'a
 * l'application.
 */
export default function PartageJeuModal({ isOpen, onClose, partie, estAuteur }: {
  isOpen: boolean
  onClose: () => void
  partie: DuoPartie
  ...
  const url = partie.shareToken ? `${boutiqueLinkOrigin()}/jeu-partie/${partie.shareToken}` : ''
  ...
  const appel = useCallback(async (chemin: string, body: Record<string, unknown>) => {
    ...
    const res = await fetch(`/api/jeux-share/${chemin}`, {
      method: 'POST',
  ...
  const retirerMembre = async (uid: string) => {
    ...
    try {
      await appel('remove-member', { partieId: partie.id, uid })
  ...
  const activer = async (): Promise<string> => {
    if (partie.shareToken) return `${boutiqueLinkOrigin()}/jeu-partie/${partie.shareToken}`
    setWorking(true); setError('')
    try {
      const token = await activerPartageLien(partie.id)
      return `${boutiqueLinkOrigin()}/jeu-partie/${token}`
    } catch {
  ...
  const couper = async () => {
    setWorking(true); setError('')
    try { await couperPartageLien(partie.id) }
  ...
  const partager = async () => {
    try {
      await navigator.share({ title: `Partie de ${partie.jeu}`, text: 'Suis les scores :', url })
    } catch { /* annulé */ }
  }

  // ── Envoi à une adresse email ──────────────────────────────────────────────
  const [email, setEmail] = useState('')
  /** Adresse dont le retrait attend une confirmation en ligne. */
  const [aRetirer, setARetirer] = useState<string | null>(null)
  const emails = useMemo(() => partie.sharedEmails ?? [], [partie.sharedEmails])
  ...
  const envoyerParEmail = async () => {
    ...
    try { await noterEmailPartage(partie.id, adresse) } catch { /* mémo seulement */ }
    setEmail('')

    // Comme partout dans l'app : le courrier part de la messagerie de l'utilisateur.
    const sujet = `Partie de ${partie.jeu}`
    const corps = [
      'Bonjour,',
      '',
      `Je te partage notre partie de ${partie.jeu} : tu peux suivre les scores et les saisir,`,
  ...
  const rattacher = async (u: User) => {
    ...
    try {
      await appel('add-member', { partieId: partie.id, uid: targetUid })
      setRecherche('')
      await chargerMembres()
      ...
      <div className="space-y-5">
        ...
        <div className="space-y-3">
          ...
          <p className="text-sm text-gray-500">
            N&apos;importe quel appareil autour de la table peut ouvrir le lien (ou scanner le QR) pour
            voir et compléter la partie. Sans compte, on ne peut pas créer de nouvelles parties.
          </p>
          ...
          {!partie.shareToken ? (
            ...
            <>
              ...
              <div className="flex items-center gap-2 bg-rose-50 border border-rose-100 rounded-xl px-3 py-2">
                ...
                <span className="text-xs text-rose-900 truncate flex-1">{url}</span>
              </div>
        ...
        <div className="border-t border-dashed border-gray-200 pt-4 space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={envoyerParEmail} disabled={working || !emailValide(email)}
              className="shrink-0 flex items-center gap-1.5 bg-rose-600 hover:bg-rose-700 disabled:opacity-40 text-white text-sm font-medium px-3 py-2 rounded-xl transition">
          ...
          {emails.length > 0 && (
            <div className="flex flex-wrap gap-1.5 pt-1">
              {emails.map((e) => (
                <span key={e} className={`inline-flex items-center gap-1 text-xs rounded-full pl-2.5 pr-1 py-1 ${
                  aRetirer === e ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-gray-100 text-gray-600'
                }`}>
                  {aRetirer === e ? 'Retirer ?' : e}
                  {aRetirer === e ? (
                    <>
                      {/* Confirmation EN LIGNE : une seconde modale par-dessus
                          celle-ci se disputerait le verrou de défilement. */}
                      <button onClick={() => { retirerEmailPartage(partie.id, e); setARetirer(null) }}
                        aria-label={`Confirmer le retrait de ${e}`}
                        className="p-0.5 text-red-600 hover:text-red-700 transition">
                        <Check size={13} />
                      </button>
                      <button onClick={() => setARetirer(null)} aria-label="Annuler"
                        className="p-0.5 text-gray-400 hover:text-gray-700 transition">
                        <X size={13} />
                      </button>
                    </>
                  ) : (
                    <button onClick={() => setARetirer(e)} aria-label={`Retirer ${e} de la liste d'envoi`}
        ...
        {isAdmin && (
          <div className="border-t border-dashed border-gray-200 pt-4">
                      ...
                      return (
                        <div key={uid} className="flex items-center justify-between gap-3 px-3 py-2">
                          ...
                          <button onClick={() => rattacher(u)} disabled={!!ajoutUid}
                            className="shrink-0 text-xs font-medium bg-rose-600 hover:bg-rose-700 disabled:opacity-60 text-white px-3 py-1.5 rounded-lg transition">
            ...
            <p className="text-xs text-gray-400 mt-2">
              Le compte rattaché retrouve la partie dans son app « Sarah &amp; Ted », en accès gratuit.
